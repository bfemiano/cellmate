...
/**
 * Basic extractor implementation that offers cell filter capability by provided
 * Predicate rules.</br>.
 * 
 * This class contains methods that look through a cell group and return one or
 * more cell(s) that match a given label. Specifically, there are methods that
 * expect to find a single cell matching the label before returning the value.
 * 
 ...
 * additioanlly annoting your cell class with {@link cellmate.cell.Label} and
 * {@link cellmate.cell.Value}.
 * 
 * {@link CellReflector}
 */
@Beta
public class SingleMultiValueCellExtractor implements CellExtractor {
	...
	/**
	 * Reads a list of cells and returns a filtered list containing only those
	 * with the matching label.
	 * 
	 * @param cells
	 *            list of cells to filter
	 * @param label
	 *            to filter cells on.
	 * @param <C>
	 *            cell class type
	 ...
	 *             if CellExtractionException is encountered
	 */
	public <C> Collection<C> filterCellsByLabel(List<C> cells,
			final String label) {
		return Collections2.filter(cells, new Predicate<C>() {
			public boolean apply(C c) {
				try {
					...
					throw new RuntimeException("Error during filtering "
							+ e.getType().name(), e);
				}
			}
		});
	...
	/**
	 * 
	 * Read a list of cells and return only those that return true for the given
	 * Predicate.
	 * 
	 * @param cells
	 *            list of cells to filter.
			...
			public boolean apply(C c) {
				...
				try {
					return Pattern.matches(regex,
							CellReflector.getLabelAsString(c));
				} catch (CellExtractorException e) {
					throw new RuntimeException("Error during filtering "
							+ e.getType().name(), e);
				}
	...
	/**
	 * Return a list of int values for any cells in the group matching the
	 * supplied label
	 * 
	 * @param cells
	 *            to filter.
	 ...
	 *             if errors occur during cell value reading.
	 */
	public <C> Collection<Integer> getAllIntCellValuesWithLabel(List<C> cells,
			String label) throws CellExtractorException {
	...
	/**
	 * Return a list of String values for any cells in the group matching the
	 * supplied label
	 * 
	 * @param cells
	 *            to filter.
	 ...
	 *             if errors occur during cell value reading.
	 */
	public <C> List<String> getAllStringCellValuesWithLabel(List<C> cells,
			String label) throws CellExtractorException {
		Collection<C> values = filterCellsByLabel(cells, label);
	...
	/**
	 * Return a list of long values for any cells in the group matching the
	 * supplied label
	 * 
	 * @param cells
	 *            to filter.
	 ...
	 * @throws CellExtractorException
	 *             if errors occurs while reading cell value, including multiple
	 *             values found.
	 */
	 ...
	 * @return long value.
	 * @throws CellExtractorException
	 *             if error occurs while reading cell value, including multiple
	 *             values found.
	 */
	 ...
	 * @throws CellExtractorException
	 */
	public <T, C> T getTypedValueByLabel(Class<T> type, List<C> cells,
			String label) throws CellExtractorException {
		C value = getSingleCellByLabel(cells, label);
	...
	/**
	 * Assumes a list of at least one cell and returns the value from the first
	 * item in the list as an int.
	 * 
	 * 
	 * @param cells
	 *            list
	...
	/**
	 * 
	 * /** Assumes a list of at least one cell and returns the value from the
	 * first item in the list as an instance of class T.
	 * 
	 * @param <T>
	 *            value class type
	 * @param cells
	 ...
	 * @throws CellExtractorException
	 *             if error occurs while reading cell value, including multiple
	 *             values found or null value.
	 */
	public <C> String getStringValue(C cell) throws CellExtractorException {
		return CellReflector.getValueAsString(cell);
	}

	private <C> void checkForOneCell(Collection<C> cells)
			throws CellExtractorException {
		if (cells.size() > 1)
			throw new CellExtractorException(
					"Too many values for single cell get",
					ErrorType.TOO_MANY_FIELDS);
		if (cells.size() == 0)
			throw new CellExtractorException("No value for single get",
