 ...
 * Time: 11:21 PM
 */
public class AccumuloTransformeringDBResultReader<C>
        implements DBResultReader<Map.Entry<Key,Value>,C> {


    private ZooKeeperInstance zooInstance;
    private static final Logger log = Logger.getLogger(AccumuloTransformeringDBResultReader.class);
    ...
    public AccumuloTransformeringDBResultReader(AccumuloReadParameters parameters){
        try {
            String zoo = parameters.getZookeepers();
            String instanceName = parameters.getInstanceName();
            zooInstance = new ZooKeeperInstance(instanceName, zoo);
    ...
    protected Connector getConnectorFromParameters(AccumuloReadParameters parameters) {
        ...
        try {
            ...
            connector = zooInstance.getConnector(user, pass);
        } catch (NoSuchElementException e){
